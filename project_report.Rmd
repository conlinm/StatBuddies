...
1. Variable (i.e. predictor) selection using a combination of semantic grouping (some variables convey similar information) and collinearity (visually, through `pairs()` plots). 
...
1. Lastly, we apply a log transformation to BMI and other non-linear transformations to some of the predictors to build a more predictive model.
...
model_diagnostics = function(fit){
  ...
  for (i in 1:5){
    ...
    shapiro.test(residuals(fit$analyses[[i]]))$p.value 
  }
...
Now consider removing `TotChol` at a significance level $\alpha = 0.01$.
...
To determine if interaction terms might improve our model, we will perform a backward AIC stepwise() function with added `Age` interaction terms. This seemed the most plausible and made the most sense from a domain perspective. It could be understandable for instance, that alcohol use at a young age vs an older age might have cpmbined effects upon BMI.
...
final_add = lm(log(BMI) ~  Age +  AlcoholYear + SmokeNow +
  BPDiaAve + BPSysAve + TVHrsDay + CompHrsDay +
  Diabetes +  HealthGen +  LittleInterest +
  SleepHrsNight +  Race1  +  MaritalStatus +  Age:SmokeNow +  Age:BPDiaAve +  Age:BPSysAve  + Age:TVHrsDay  +  Age:Diabetes +   Age:HealthGen +  Age:Race1 +  Age:MaritalStatus, data = imp_df_1_add_rm)
```

```{r}
#Final_int Model fit with outliers removed

final_int = lm(log(BMI) ~  Age +  AlcoholYear + SmokeNow +
  BPDiaAve + BPSysAve + TVHrsDay + CompHrsDay +
  Diabetes +  HealthGen +  LittleInterest +
  SleepHrsNight +  Race1  +  MaritalStatus +  Age:SmokeNow +  Age:BPDiaAve +  Age:BPSysAve  + Age:TVHrsDay  +  Age:Diabetes +   Age:HealthGen +  Age:Race1 +  Age:MaritalStatus, data = imp_df_1_int_rm)
...
final_trns = lm(log(BMI) ~ Age +  AlcoholYear + SmokeNow +
  I(BPDiaAve^2) + log1p(BPSysAve) + TVHrsDay + CompHrsDay +
  Diabetes +  HealthGen +  LittleInterest +
  SleepHrsNight +  Race1  +  MaritalStatus +  Age:SmokeNow +  Age:I(BPDiaAve^2) +  Age:log1p(BPSysAve)  + Age:TVHrsDay +  Age:Diabetes +  Age:HealthGen +  Age:Race1 +  Age:MaritalStatus, data = imp_df_1_trns_rm)
```
...
```{r}
# bp results
row_names = c("final_add", "final_int", "final_trns")
col_names = c("Model", "BP test", "Shapiro test", "LOOCV_RMSE", "Adj R2")
...
rmse_results = c(calc_loocv_rmse(final_add), calc_loocv_rmse(final_int), calc_loocv_rmse(final_trns))
r2_results = c(get_adj_r2(final_add), get_adj_r2(final_int), get_adj_r2(final_trns))

results_table = cbind(row_names, bp_results, shapiro_results, rmse_results, r2_results)
knitr::kable(results_table, col.names = col_names, caption = "Model Comparison")
```

In summary we have been able to build a model which can explain more than 55% of the variability in the dataset, according to adjusted $R^2$.
We note, that as far as leave-one-out-cross-valitation RMSE and adjusted $R^2$ all three, the additive model, the interaction model and the transformed model are performing very similarly. All models have issues with heteroskedasticity and violation of normality assumptions according to Breusch-Pagan test and Shapiro test, respectively. 

We arrived at these models using the steps:
1. Ruling out many variables by grouping them into similar domains and keeping only one representative in case we detected collinearity. This reduced the number of possible predictors from `r ncol(NHANES)-1` to `r ncol(nhanes_select)-1`.
1. We dealt with missing data using multiple imputation using the `mice` package. 
1. We fitted a first version of an additive model with the selected predictors and detected violations of equal variance and normality. 
1. We log-transformed the response to alleviate some, but not all non-normality.
1. We performed AIC and BIC stepwise processes to reduce the number of predictors further. 
1. We tested interaction of `Age` with other predictors as well as transformations of `BPDiaAve`, `BPSysAve`. 
